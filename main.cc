...
{{if .CmdArgs}}
    ...
    {{if eq $arg1 "start"}}
        ...
        {{dbSetExpire .Channel.ID "uno" (sdict "emojis" $emojis "owner" .User.ID "chanID" .Channel.ID "mID" $mID "users" (sdict (str .User.ID) (sdict "cards" cslice))) 1800}}
    {{end}}
{{else}}
    {{sendMessage nil (cembed "title" "UNO Help" "description" "`uno start` - to start the game\n`uno p CARD` - to play that card")}}
{{end}}